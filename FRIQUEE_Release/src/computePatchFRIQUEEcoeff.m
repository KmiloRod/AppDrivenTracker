...
function FRIQUEEcoeffvect =  computePatchFRIQUEEcoeff(vidname,height,width, ...
...
for frameitr = framevector   
    ...
    [feat]                  = computefeatvect(temporalsignal,...
...
%--------------------------------------------------------------------------
function [featvect] = computeFRIQUEEfeatvector(temporalsignal)
...
temporalsignal = temporalsignal.data;
...
%                         [ blockrowoverlap blockcoloverlap],@computeFRIQUEEfeatvector);
feat                     = blockproc(structdis,[blocksizerow  blocksizecol],...
                         @computeFRIQUEEfeatvector);
                     
feat = reshape(feat',num_feats,size(feat,1)*size(feat,2)/num_feats)';
