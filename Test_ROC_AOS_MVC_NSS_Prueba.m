function  [ROC_accuracy_pristine, ROC_accuracy, AOS_threshold_array]=Test_ROC_AOS_MVC_NSS_Prueba(Distortion,v,NSS,Cs,Display,saveas, PC,Norm)
...
ROC_accuracy_pristine = linspace(0.1,1,19);
ROC_accuracy = linspace(0.1,1,19);
...
if ~strcmp(Distortion,'pristine')
    ...
    if NSS
        load(strcat('AOS_NSS_Video_',num2str(v),'_pristine_Cs',num2str(Cs),'_Norm',num2str(Norm)));
    else
        load(strcat('AOS_HOG_Video_',num2str(v),'_pristine_Cs',num2str(Cs),'_Norm',num2str(Norm)));
    end
...
%             hogS_nxt = hogFeat(I,bbox); % HOG features of the window boxes
            ...
            [features_nxt,u_old,st_old,n_patch_old]   = hogNSSFeat(I,bbox,NSS,Cs,Norm,u_old,st_old,n_patch_old);
...
%             hogS_nxt = hogFeat(I,wP);
            [features_nxt,u_old,st_old,n_patch_old] = hogNSSFeat(I,wP,NSS,Cs,Norm,u_old,st_old,n_patch_old);            
        ...
        if ((sum(isnan(objP_nxt))==0) & (isempty(objP_nxt)==0))
            ...
            if flag(i) == 0
                    ...
                    if NSS
                        [hogS,~,nssS] = bgUpdateNSS(I,'mvc',0.05,objbbox(i,:),bgP,bgKeys,hogS,nssS,Cs,Norm,u_old,st_old,n_patch_old);
                        if PC
                                hogS=PCA_(hogS,hogpca);
                                nssS=PCA_(nssS,hogpca);
                        end
                    else
                        [hogS,u_old,st_old,n_patch_old] = bgUpdate(I,'mvc',0.05,objbbox(i,:),bgP,bgKeys,hogS,Norm,u_old,st_old,n_patch_old);
                        if PC
                                hogS=PCA_(hogS,hogpca);
                        end
                    end
...
if saveas
    ...
    if NSS
        if strcmp(Distortion,'S & P')
            save(strcat('AOS_NSS_Video_',num2str(v),'_S_P_Cs',num2str(Cs),'_Norm',num2str(Norm)),'predict_accuracy','AOS','ROC_accuracy','false_p','fs','total_object_found','flag_lvl');
        else
            save(strcat('AOS_NSS_Video_',num2str(v),'_',Distortion,'_Cs',num2str(Cs),'_Norm',num2str(Norm)),'predict_accuracy','AOS','ROC_accuracy','false_p','fs','total_object_found','flag_lvl');
        end
    else
        if strcmp(Distortion,'S & P')
            save(strcat('AOS_HOG_Video_',num2str(v),'_S_P','_Norm',num2str(Norm)),'predict_accuracy','AOS','ROC_accuracy','false_p','fs','total_object_found','flag_lvl');
        else
            save(strcat('AOS_HOG_Video_',num2str(v),'_',Distortion,'_Cs',num2str(Cs),'_Norm',num2str(Norm)),'predict_accuracy','AOS','ROC_accuracy','false_p','fs','total_object_found','flag_lvl');
        end
    end
